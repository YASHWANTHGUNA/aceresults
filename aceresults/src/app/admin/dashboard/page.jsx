...
export default function AdminDashboard() {
  ...
  const deleteRecord = async () => {
    if (!confirm(`WARNING: Are you sure you want to permanently delete the records for ${studentData.rollNumber}?`)) {
      return;
    }
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 p-8 font-sans">
      ...
      <div className="flex justify-between items-center border-b border-slate-800 pb-6 mb-10 max-w-6xl mx-auto">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-wide">Admin Control Center</h1>
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 sticky top-8">
            ...
            <form onSubmit={searchStudent} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">Student Roll Number</label>
                <input
                  type="text"
            ...
            {error && <div className="mt-4 text-rose-400 text-sm bg-rose-400/10 p-3 rounded border border-rose-400/20">{error}</div>}
            {message && <div className="mt-4 text-emerald-400 text-sm bg-emerald-400/10 p-3 rounded border border-emerald-400/20">{message}</div>}
          </div>
        </div>
          ...
          {studentData ? (
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden shadow-2xl">
              ...
              <div className="bg-slate-900 p-6 border-b border-slate-700 flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-bold text-white tracking-widest">{studentData.rollNumber}</h3>
                  <p className="text-sm text-slate-400">Semester: <span className="text-white font-semibold">{studentData.semester}</span></p>
                </div>
              ...
              <div className="p-6 grid grid-cols-2 gap-4 border-b border-slate-700 bg-slate-800/30">
                <div className="p-4 bg-slate-900 rounded-lg border border-slate-700 text-center">
                  ...
                  <p className="text-2xl font-bold text-white">{studentData.sgpa?.toFixed(2) || "N/A"}</p>
                </div>
                <div className="p-4 bg-slate-900 rounded-lg border border-slate-700 text-center">
                  <p className="text-sm text-slate-400 mb-1">Status</p>
                  <p className={`text-2xl font-bold ${studentData.status === "PASS" ? "text-emerald-400" : "text-rose-400"}`}>
                    {studentData.status}
                  </p>
                </div>
              </div>

              {/* Detailed Subjects Table (Admin View) */}
              <div className="p-6 overflow-x-auto border-b border-slate-700">
                <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wider mb-4">Granular Subject Data</h3>
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-slate-300 text-sm">
                    {studentData.subjects.map((sub, index) => {
                      const raw = sub.rawString || sub.rawExtractedString || sub.subjectCode || "";
                      const parts = raw.trim().split(/\s+/);
                      
                      let credits = "-", grade = "-", code = sub.subjectCode, name = "-";
                      
                      if (parts.length > 4) {
                        credits = parts.pop();
                        parts.pop(); // discard redundant Pass/Fail status
                        grade = parts.pop();
                        
                        // --- THE FIX ---
                        // If the first chunk of text is the Roll Number, remove it!
                        if (parts[0] === studentData.rollNumber) {
                          parts.shift(); 
                        }
                        
                        code = parts.shift(); // Now safely grab the Subject Code
                        name = parts.join(" ");
                      } else {
                        name = raw; 
                      }
                      ...
                      return (
                        <tr key={index} className="border-b border-slate-700/50 hover:bg-slate-700/20 transition-colors">
                          <td className="py-2 px-3 font-mono text-blue-400 whitespace-nowrap text-xs">{code}</td>
                          <td className="py-2 px-3 font-medium text-xs">{name}</td>
                          <td className={`py-2 px-3 text-center font-bold text-xs ${grade.includes('F') ? 'text-rose-400' : 'text-emerald-400'}`}>
                            {grade}
                          </td>
              ...
              <div className="p-6 bg-rose-950/20">
                ...
                <p className="text-sm text-slate-400 mb-4">
                  Deleting this record will permanently remove it from the database. The student will immediately lose access to these results.
                </p>
                <button
                ...
                >
                  {actionLoading ? "Processing Deletion..." : "Permanently Delete Record"}
                </button>
              </div>
            </div>
          ) : (
            <div className="h-full flex items-center justify-center border-2 border-dashed border-slate-700 rounded-xl p-12 text-center text-slate-500">
              <div>
                ...
                <p>Search for a student roll number to view and manage their detailed T-Sheet data.</p>
              </div>
            </div>
          )}
        </div>
